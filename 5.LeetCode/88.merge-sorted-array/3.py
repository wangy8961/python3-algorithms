...
3. 依次类推。直到 nums1 前半段(下标为 0 至 m-1) 或 nums2 其中一个序列为空时：
    (1) 如果 nums2 先为空，因为原始 nums1 就是有序的，所以剩余的下标 0 至 i 的元素还是有序的，且肯定比结果数组的小，所以无需再迭代复制！
    (2) 如果 nums1 前半段先为空，再把 nums2 的剩余数据依次往前复制到结果数组即可
