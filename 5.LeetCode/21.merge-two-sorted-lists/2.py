...
1. 判断 l1 和 l2 第一个节点哪个更小，然后较小节点的 next 指针指向「其余节点的合并结果(递归调用)」
...
或者这样分析，递归函数每次去掉一个元素，直到两个链表都为空，因此需要调用 R=O(m+n) 次。
...
class Solution:
    def mergeTwoLists(self, l1: ListNode, l2: ListNode) -> ListNode:
        ...
        if l1.val <= l2.val:
            l1.next = self.mergeTwoLists(l1.next, l2)  # 较小节点 l1 的 next 指针指向「其余节点的合并结果」
            return l1  # 返回合并后链表的第一个节点
        else:
            l2.next = self.mergeTwoLists(l1, l2.next)  # 较小节点 l2 的 next 指针指向「其余节点的合并结果」
