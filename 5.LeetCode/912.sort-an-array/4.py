...
class Solution:
    def merge(self, nums: List[int], low: int, mid: int, high: int):
        """合并左右两个「有序」子数组
        左子数组 nums1 下标 low 至 mid
        右子数组 nums2 下标 mid + 1 至 high
        """
        temp = []  # 临时结果数组，大小为 high - low + 1
        i, j = low, mid + 1  # 分别表示 nums1 和 nums2 数组中当前迭代的数据的下标

        while i <= mid and j <= high:
            if nums[i] <= nums[j]:  # 如果 nums1 中的值较小时
                temp.append(nums[i])
                i += 1
        ...
        if i <= mid:  # nums1 可能还有剩余的数据
            for p in range(i, mid + 1):
                temp.append(nums[p])
        if j <= high:  # nums2 可能还有剩余的数据
