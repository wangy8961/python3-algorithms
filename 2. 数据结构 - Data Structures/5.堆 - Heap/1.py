"""
Python 语言可以直接使用 heapq 模块构建小顶堆：heapq.heapify
我们这里使用数组来存储堆中元素，实现一个简单的小顶堆，支持插入元素(heappush)，弹出堆顶元素(heappop)，更新堆顶元素(heapreplace)，原地堆化数组(heapify)

注意：「堆化」操作只针对原来就是有效堆结构，当堆顶元素改变了(_siftdown)、或者在堆尾添加了新元素(_siftup)时，才能调用这两个堆化函数！
分为从上往下和从下往上堆化，因为单个元素的变化所以需要调整结构，让其继续符合小顶堆的特性，将较小的元素依次交换到父节点，堆顶元素是最小的
"""
...
class MinHeap:
    ...
    def heappop(self):
        """删除堆顶元素
        如果直接删除堆顶元素，就需要把第二小的元素(肯定在它的左、右节点中)放到堆顶。然后再迭代地删除第二小节点，以此类推，直到叶子节点被删除。这种方法有点问题，就是最后堆化出来可能出现数据空洞，就会不满足完全二叉树的特性

        正确方法：
        1. 先把堆顶元素保存到临时变量中，用于最后返回
        2. 删除堆的最后一个元素，并将它的值更新为新的堆顶元素
        3. 对新堆顶元素，使用「自顶向下」进行堆化。将它与左、右节点比较大小，如果左节点或右节点比它小，则交换它们
        4. 以此类推，直到父子节点不再需要交换
        """
        if len(self.heap) == 0:  # 空堆
    ...
    def heapify(self, nums: List[int]) -> None:
        """堆化：「原地」调整 nums 数组，让其满足小顶堆的特性
        假设存储堆的数组 nums 从下标 0 开始计数，元素个数为 n。那么下标为 i 的元素的左孩子下标为 2i + 1，右孩子下标为 2i + 2
        「非叶子节点」的最大的下标为 n//2 - 1，所以我们只需要「自底向上、自右向左」依次针对每个非叶子节点进行「自顶向下」堆化，让其满足小顶堆的特性即可
        """
        self.heap = nums
        n = len(nums)
    ...
    def _siftup(self, start: int, end: int):
        """
        「自底向上」进行堆化
        注意：除下标 start 节点以外，其它节点已经是一个小顶堆了！
        顺着该节点到根节点所在的路径，向上依次与父节点比较，如果它比父节点小，则交换它们
        ...
        while start > end:
            ...
            if self.heap[start] < self.heap[parent]:  # 如果新插入的节点比父节点小，则交换它们
                self.heap[start], self.heap[parent] = self.heap[parent], self.heap[start]
                start = parent
            else:  # 不用再向上比较了，上面的祖父节点都比父节点小
                break
    ...
    def _siftdown(self, start: int, end: int):  # 「迭代」
        ...
        「自顶向下」进行堆化
        注意：除下标 start 节点以外，其它节点已经是一个小顶堆了！
        顺着该节点到叶子结点所在的路径，向下依次与它的左、右节点比较，如果左、右节点比它小，则交换它们
